...
vi.mock("ai", () => ({
  generateObject,
  NoObjectGeneratedError: { isInstance: (error: unknown) => error instanceof Error && error.name === "NoObjectGeneratedError" },
}));

const input: LenientExtraction = {
  patient: { firstName: "  Ada   ", lastName: "  okafor ", dob: "3/12/1985", mrn: "mrn-1234567 ", phone: "(555) 555-0101" },
  encounter: { date: "02-20-2026", type: "telehealth", providerName: " Dr.   Yu ", npi: "109 381 7465" },
...
describe("normalizeExtraction", () => {
  it("normalizes whitespace, dates, codes, and numeric identifiers", () => {
    expect(normalizeExtraction(input)).toEqual({
      extraction: {
        patient: { firstName: "Ada", lastName: "okafor", dob: "1985-03-12", mrn: "MRN-1234567", phone: "(555) 555-0101" },
        encounter: { date: "2026-02-20", type: "telehealth", providerName: "Dr. Yu", npi: "1093817465" },
        diagnoses: [{ description: "Fever unspecified", icd10: "R50.9" }],
        lines: [{ description: "Lipid panel", cpt: "80061", units: 1, chargeCents: 5000 }],
        payer: { name: "Cigna", memberId: "KQM1" },
        printedTotalCents: 5000,
  ...
  it("preserves ISO dates and nullable codes", () => {
    const result = normalizeExtraction({ ...input, patient: { ...input.patient, dob: "1985-03-12" }, encounter: { ...input.encounter, date: "2026-02-20" }, diagnoses: [{ ...input.diagnoses[0], icd10: null }], lines: [{ ...input.lines[0], cpt: null }] });
  ...
  it("leaves an unrecognized date shape unchanged", () => {
    expect(normalizeExtraction({ ...input, patient: { ...input.patient, dob: "not-a-date" } }).extraction?.patient.dob).toBe("not-a-date");
  });

  it("returns normalized model output and usage", async () => {
    getLanguageModel.mockReturnValue({ model: "fake-model", modelId: "fake-id" });
    generateObject.mockResolvedValue({ object: input, usage: { inputTokens: 12, outputTokens: 7 } });
